...
def many_to_many_temp(documents, sections_documents):
    return [(document.name, sec_documents.document_id, sec_documents.section_id)
                         for document in documents
...
def many_to_many(documents, sections):
    return [(sec.name, sec.page, document_name)
                    for document_name, document_id, sec_id in many_to_many_temp(documents, sections_documents)
...
def task_3(documents, sections):
    mas_list = []
    for folder_name, i, document_name in many_to_many(documents, sections):
